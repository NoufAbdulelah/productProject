...
export class TestUtils {
   ...
   */
  getProduct(entityName) {
    const order: string[] = JSON.parse(
      fs.readFileSync(Path.join(__dirname, "../test/fixtures/_Product.json"), "utf8")
    )
  ...
  async getEntities() {
    ...
    ;(await (await this.databaseService.connection).entityMetadatas).forEach(x =>
      entities.push({ name: x.name, tableName: x.tableName, order: this.getProduct(x.name) })
    )
    return entities
  }
